...
class TransfersRegistrationData {
  final TransfersRegistrationRepository repository;

  TransfersRegistrationData(this.repository);

  Future<List<TransfersRegistrationModel>?> call() async {
    final transfersRegistrationData = await repository.loadTransfersRegistrationData();

    for (var transfer in transfersRegistrationData!) {
  ...
  void _validateTransfer(TransfersRegistrationModel transfer) {
    ...
    final minDate = DateTime.now().subtract(const Duration(days: 365 * 10)); // No más de 10 años atrás
